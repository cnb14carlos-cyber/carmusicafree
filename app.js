...
const previewResolveCache = new Map();
...
nativeAudio.onpause = () => {
  if (currentRadioId === null && nativeAudio.ended) return;
  if (currentRadioId === null && (!ytPlayer || !isYtReady)) {
    isPlaying = false;
    syncPlayerUI();
  }
};
...
nativeAudio.ontimeupdate = () => {
  if (currentRadioId !== null || !nativeAudio.duration || !Number.isFinite(nativeAudio.duration)) return;
  progressPercent = Math.min(100, ((nativeAudio.currentTime || 0) / nativeAudio.duration) * 100);
  syncPlayerUI();
};
...
async function findOfficialPreviewForSong(song) {
  if (!song) return '';
  if (song.previewUrl) return song.previewUrl;

  const cacheKey = normalizeSearchText(`${song.artist} ${song.title}`);
  if (previewResolveCache.has(cacheKey)) return previewResolveCache.get(cacheKey);

  try {
    const data = await searchMusicFromBrowser(`${song.artist} ${song.title}`);
    const normalizedTitle = normalizeSearchText(song.title);
    const normalizedArtist = normalizeSearchText(song.artist);
    const match = (data.tracks || []).find((track) => {
      ...
      return track.previewUrl
        ...
        && (normalizedArtist.includes(trackArtist) || trackArtist.includes(normalizedArtist.split(' ')[0]));
    }) || (data.tracks || []).find((track) => track.previewUrl);

    const previewUrl = match?.previewUrl || '';
    if (match) {
      song.previewUrl = previewUrl;
      song.cover = match.cover || song.cover;
      song.album = match.album || song.album;
      song.time = match.time || song.time;
      song.isPreview = true;
      song.source = match.source || 'itunes';
      addOrUpdateOnlineResults([song], []);
    }
    previewResolveCache.set(cacheKey, previewUrl);
    return previewUrl;
  } catch (err) {
    console.warn('Preview resolve note:', err);
    previewResolveCache.set(cacheKey, '');
    return '';
  }
}
...
function syncPlayerUI() {
  if (currentRadioId !== null) {
    ...
    if (song) {
      ...
      const totalSeconds = (nativeAudio.duration && Number.isFinite(nativeAudio.duration))
        ? nativeAudio.duration
        : parseDurationToSeconds(song.time || '0:30');
    ...
    if (currentRadioId !== null) {
      ...
      if (radio) {
        ...
        nativeAudio.play().then(() => {
          ...
          isPlaying = false;
          syncPlayerUI();
          showToast('El navegador ha bloqueado el audio. Toca otra vez en reproducir.');
        });
        showToast(`📻 Sintonizando en directo: ${radio.name}`);
      }
    } else {
      playSong(songs[currentSongIndex]?.id ?? currentSongIndex);
      return;
    }
  } else {
    isPlaying = false;
...
async function playSong(songIdentifier) {
  ...
  isPlaying = false;
  syncPlayerUI();
  showToast(`Cargando: ${song.title} (${song.artist})`);
  ...
  const previewUrl = await findOfficialPreviewForSong(song);
  if (currentSongIndex !== targetIndex || currentRadioId !== null) return;

  if (!previewUrl) {
    isPlaying = false;
    syncPlayerUI();
    showToast(`No hay preview disponible para "${song.title}". Prueba otra canción.`);
    return;
  }

  nativeAudio.src = previewUrl;
  nativeAudio.loop = false;
  setNativeVolume(currentVolume);
  ...
  try {
    ...
    if (currentSongIndex !== targetIndex || currentRadioId !== null) {
      nativeAudio.pause();
      return;
    }
    isPlaying = true;
    showToast(`▶ Reproduciendo preview oficial: ${song.title}`);
  } catch (err) {
  ...
  progressTimer = setInterval(() => {
    ...
    if (nativeAudio.duration > 0 && Number.isFinite(nativeAudio.duration)) {
...
function initEvents() {
  ...
  $('#progress').oninput = (e) => {
    ...
    if (nativeAudio.duration && Number.isFinite(nativeAudio.duration)) {
      nativeAudio.currentTime = (progressPercent / 100) * nativeAudio.duration;
    }
    syncPlayerUI();
  };

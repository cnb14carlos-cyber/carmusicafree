...
function bindEvents() {
  ...
  el.audio.addEventListener("playing", () => {
    if (state.current) el.playerArtist.textContent = labelFor(state.current);
    startProgressTimer();
  });
  ...
  el.audio.addEventListener("error", () => {
    stopProgressTimer();
    el.playButton.textContent = "Play";
    el.playerArtist.textContent = "Esta preview no esta disponible. Prueba otro resultado.";
  });
...
async function playTrack(item, queueIndex = -1) {
  ...
  updatePlayer(item);
  el.playerArtist.textContent = "Cargando audio...";
  ...
  try {
    ...
    if (state.loadToken !== loadToken) return;
    el.playButton.textContent = "Play";
    el.playerArtist.textContent = "Pulsa Play otra vez para activar el audio";
  }
}
...
function togglePlay() {
  ...
  if (el.audio.paused) {
    if (state.current && (state.current.type === "radio" || state.current.resultType === "radio")) {
      ...
      el.audio.play().catch(() => {
        el.playerArtist.textContent = "Pulsa otra vez para activar el audio";
      });
    }
  } else {
    el.audio.pause();
